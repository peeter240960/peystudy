...
app.post('/login', async (req, res) => {
    ...
    const path = `${__dirname}/public/pdf/${user.sid}.pdf`
    if (!fs.existsSync(path) && user.role != 'admin') {
        const message = await Message.findOne({})
        res.status(401).json({
            success: false,
            message: `<p class="font-bold">${message.message}</p>`
        })
        return
    }
...
app.post('/admin/messages', [authen, admin], async (req, res) => {
    let { id } = req.query
    let message
    if (id && id != 'null') {
...
app.post('/admin/student_create', authen, async (req, res) => {
    ...
    for (let i = 0; i < students.length; i++) {
        ...
        try {
            checkStudent = await User.findOne({ _id: student._id })
...
app.get('/download', authen, async (req, res) => {
    ...
    let file = fs.readFileSync(`${path}/${user.sid}.pdf`)
    let stat = fs.statSync(`${path}/${user.sid}.pdf`);
...
app.get('/check_has_file', authen, async (req, res) => {
    let path = __dirname + '/public/pdf'
    const { userId } = req.authen
    const user = await User.findOne({ _id: userId })
    if (!user) {
