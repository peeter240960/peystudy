<template>
  ...
  >
    ...
    <section>
      ...
      <form @submit.prevent="saveMessage" class="flex mt-5">
        <input
          type="text"
          name="error"
          id="error"
          ...
          v-model="error"
        />
        <button class="bg-blue-400 p-2">Save</button>
      </form>
      <div></div>
    </section>
...
export default {
      ...
      if (val) {
        this.intervalServerProcess = setInterval(async () => {
          try {
            ...
            this.loading = data.result.active;
            this.serverProcessed =
              (data.result.current / data.result.max) * 100;
            if (data.result.active === false || this.serverProcessed === 100) {
              clearInterval(this.intervalServerProcess);
              console.log("interval --> " + this.intervalServerProcess);
              this.intervalServerProcess = null;
  ...
  methods: {
    async saveMessage() {
      try {
        await this.$axios.$post("/api/admin/messages?id=" + this.messageId, {
          message: this.error,
        });
        this.getMessage();
      } catch (err) {
    ...
    async getMessage() {
      try {
        const resp = await this.$axios.$get("/api/messages");
        this.error = resp.result[0].message;
        this.messageId = resp.result[0]._id;
      } catch (err) {
        if (err?.response?.status == 400) {
          Swal.fire({
            icon: "error",
